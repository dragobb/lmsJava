...
public class LogIn extends JFrame {
    ...
    private void verifyLogin() throws SQLException {
        ...
        try {
            // Ensure driver is available. Avoid direct reference to DriverLoader to prevent
            // NoClassDefFoundError when runtime classes are out-of-date. Try in order:
            // 1) call DriverLoader.ensureDriver() reflectively if class is present
            // 2) Class.forName("com.mysql.cj.jdbc.Driver")
            // 3) load mysql-*.jar from ./libs via URLClassLoader and register driver
            ...
            try {
                ...
                // DriverLoader class not present at runtime; fall through
            } catch (Exception ex) {
                // invocation problem - continue to other attempts
            }
            ...
            showErrorDialog("Driver Error", "MySQL JDBC driver not found: " + e.getMessage());
        }
    }
    ...
    public static void main(String[] args) {
        // Try to ensure the JDBC driver is available early so IDE-run launches
        try {
            try {
                Class<?> dl = Class.forName("bonifacio_bscs1a.libmanagement.DriverLoader");
                dl.getMethod("ensureDriver").invoke(null);
            } catch (ClassNotFoundException cnf) {
                // DriverLoader not present; fall through
            } catch (Exception ex) {
                // ignore failures here and let verifyLogin attempt fallbacks
            }
        } catch (Throwable t) {
            // ignore
        }
