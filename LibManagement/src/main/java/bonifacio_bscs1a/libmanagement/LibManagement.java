                    ...
                    if (innerComp instanceof JPanel innerPanel) {
                        if (innerPanel.getLayout() instanceof BorderLayout) {
                            for (Component centerComp : innerPanel.getComponents()) {
                                if (centerComp instanceof JScrollPane scrollPane) {
...
if (available) {
   ...
   actionButton.addActionListener((ActionEvent e) -> {
    ...
    private void loadBooks() {
        ...
        // The table model doesn't exist because we're using the new card UI
        // Just find the container and reload the cards
        JPanel container = findBookCardsContainer();
        if (container != null) {
            loadModernBookCards(container);
        }
